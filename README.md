...
> [!CAUTION]
> **使用提示：** 本项目用于 ESP32、传感器和局域网 HTTP 学习，不可用于婴儿看护、医疗、睡眠安全、紧急响应或其他生命安全场景。
...
完整的 [BOM](hardware/BOM.csv)、[接线图](hardware/wiring-diagram.svg)和[硬件说明](HARDWARE.md)列出了项目接口。首次接线、改线或烧录前请断电，确认电压、电流、电平、限流、供电能力和公共地；不要把 5 V 信号直接接入 ESP32 GPIO，也不要用 GPIO 直接驱动超出额定电流的负载。
...
| 方法 | 路径 | 用途 |
| :-- | :-- | :-- |
| `GET` | `/api/status` | 读取本次传感器数据与提示等级。 |
| `POST` | `/api/mute` | 切换蜂鸣器静音状态。 |
